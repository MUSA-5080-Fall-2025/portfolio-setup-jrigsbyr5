...
author: "Elizabeth Delmelle, Contributor: Joshua Rigsby"
date: today
format: 
  html:
    code-fold: false
...
county_data <- county_data %>%
  mutate(county_name = str_remove(NAME, paste0(", ", "PA")))
...
ggplot(county_data) +
  ...
  labs(
    title = "Distribution of Total Poulation",
    x = "Population",
    y = "Number of Counties"
  ) +
  theme_minimal() +
...
# Boxplot of Total Population to identify outliers
ggplot(county_data) +
  aes(y = total_popE) +
  geom_boxplot(fill = "purple", width = 0.5) +
  labs(
    title = "Total Poulation Distribution with Outliers",
    y = "Population"
  ) +
  theme_minimal() +
  scale_y_continuous(labels =)

# Find and list the most and least populous three counties - REVIEW
...
county_reliability_age <- county_reliability %>%
  mutate(age_moe_pct = (median_ageM / median_ageE) * 100)

ggplot(county_reliability_age) +
  aes(x = total_popE, y = age_moe_pct) +
  geom_point(alpha = 0.7, color = "purple") +
  geom_hline(yintercept = 10, color = "blue", linetype = "dashed") +
  labs(
    title = "Data Reliability Age Margin of error",
    x = "Total Population",
    y = "Age Margin of Error (%)",
    caption = "Blue line = 10% reliability threshold"
  ) +
  theme_classic() +
  scale_x_continuous(labels = comma)
...
```{r}
county_income_age = county_data %>%
  mutate(
    income_category = case_when(
      median_incomeE < 50000 ~ "Low",
      median_incomeE <= 80000 ~ "Middle",
      TRUE ~ "High"
    )
  )

# Scatter plot
ggplot(county_income_age, aes(x = total_popE, y = median_ageE)) +
  geom_point(aes(color = income_category, size = median_incomeM), alpha = 0.7) +
...
education_data = get_acs(
  ...
  mutate(
    ...
    county_name = str_remove(NAME, paste0(", ", "PA"))
  ) %>%
