...
acs_tracts = acs_tracts %>%
  mutate(
    income_category = case_when(
      ...
      median_incomeE >= 40000 & median_incomeE <= 60000 ~ "Moderate",
...
**Questions to answer:**
- What income threshold did you choose and why? I chose under $40000 to designate low median household income, because its measured by median not mean so there will be true population values in the tract above and below this number not represented by the median household income. 
...
- How many tracts meet your vulnerability criteria? 35
- What percentage of PA census tracts are considered vulnerable by your definition? 1.01%
---
...
**Questions to answer:**
- How many tracts are underserved? 0
- What percentage of vulnerable tracts are underserved? 0
- Does this surprise you? Why or why not? Yes, this is surprising, perhaps its due to the parameters I used to define vulnerable but, I would think that especially in rural tratcts in Pennsylvania that there would be a percentage of vulnerable people more than 15 miles from the nearest hospital.
---
...
**Questions to answer:**
- Which 5 counties have the highest percentage of underserved vulnerable tracts? None/NA
- Which counties have the most vulnerable people living far from hospitals? NORTHUMBERLAND,   ALLEGHENY, CAMBRIA, LEHIGH
- Are there any patterns in where undeserved counties are located? I did not have any undeserved counties but I would imagine that patterns could be counties located in more rural areas.
...
priority_counties <- c(
  "ALLEGHENY", "CAMBRIA", "ERIE", "MERCER", "PHILADELPHIA",
  "WESTMORELAND", "LEHIGH", "NORTHUMBERLAND", "BEAVER", "LUZERNE"
)
...
ggplot() +
  ...
  geom_sf(data = pa_hospitals_2272, shape = 21, fill = "blue", size = .25) +
...
# If I had tracts with underserved populations it wouls how up, but I dont so its more blank
```
...
# If I had tracts with undeserved populations it wouls how up, but I dont so its more blank
